...
def make_table(names, datasets, data_loaders, model, latex=False):
    ...
    table.add_column("Mean Error (km)", justify="right")
    table.add_column("Median Error (km)", justify="right")
    table.add_column("Correct Country", justify="right")
    table.add_column("Correct Feature Code", justify="right")
    table.add_column("Correct ADM1", justify="right")
    table.add_column("Missing correct", justify="right")
    table.add_column("Total missing", justify="right")
    table.add_column("Acc @161km", justify="right")
...
def main(data_dir: Path, path: Path):
    wandb.init()
    ...
    _, _, gwn_loader_full, es_data_gwn_full = load_data(
        data_dir,
        max_results=config.max_choices,
        fuzzy=config.fuzzy,
        limit_types=config.limit_es_results,
    ...
    prod_loader = DataLoader(
        dataset=prod_data, batch_size=config.test_batch_size, shuffle=False
    )
    tr_loader = DataLoader(
        dataset=tr_data, batch_size=config.test_batch_size, shuffle=False
    )
    lgl_loader = DataLoader(
        dataset=lgl_data, batch_size=config.test_batch_size, shuffle=False
    )
    gwn_loader = DataLoader(
        dataset=gwn_data, batch_size=config.test_batch_size, shuffle=False
    )
    gwn_full_loader = DataLoader(
        dataset=gwn_full_data, batch_size=config.test_batch_size, shuffle=False
    )
    syn_loader = DataLoader(
        dataset=syn_data, batch_size=config.test_batch_size, shuffle=False
    )
    wiki_loader = DataLoader(
        dataset=wiki_data, batch_size=config.test_batch_size, shuffle=False
    )
