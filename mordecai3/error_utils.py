...
def evaluate_results(es_data, loader, model):
    ...
    for ent, pred in zip(es_data, pred_array):
        ...
        predicted_position = np.argmax(
            [i["score"] for i in ent["es_choices"] if "score" in i.keys()]
        )
        ...
        if (
            correct_position == len(ent["es_choices"]) - 1
            and predicted_position == len(ent["es_choices"]) - 1
        ):
            total_missing += 1
            missing_correct += 1
            continue
        elif (
            correct_position is None
            and predicted_position == len(ent["es_choices"]) - 1
        ):
            total_missing += 1
            missing_correct += 1
    ...
    if total_missing > 0:
        ...
        miss_correct_perc = 0
    correct_avg = {
        "correct_country": np.mean(correct_country),
