...
def token_tensors(doc):
    ...
    for n, i in enumerate(doc):
        ...
        for d in wordpiece_num.dataXd:
            ...
            # print(n, i, wordpiece)
            token_tensors[n] = token_tensors[n] + [
                doc._.trf_data.tensors[0][which_chunk][which_token]
            ]
...
@st.cache(allow_output_mutation=True, suppress_st_warning=True)
def load_model():
    model = geoparse_model(device=-1, bert_size=768, num_feature_codes=54)
...
def load_geo():
    geo = Geoparser(
        model_path="mordecai_2023-02-07_good.pt",
