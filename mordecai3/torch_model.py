...
class ProductionData(Dataset):
    def __init__(self, es_data, max_choices=25, max_codes=50):
        ...
        self.placename_tensor = np.array([i["tensor"] for i in es_data]).astype(
            np.float32
        )
        self.doc_tensor = np.array([i["doc_tensor"] for i in es_data]).astype(
            np.float32
        )
        self.other_locs_tensor = np.array([i["locs_tensor"] for i in es_data]).astype(
            np.float32
        )
    ...
    def create_feature_codes(self, es_data):
        ...
        for ex in es_data:
            feature_code_raw = [
                i["feature_code"] for i in ex["es_choices"][0 : self.max_choices]
            ]
            ...
            feature_codes = [
                self.feature_code_dict[i]
                if i in self.feature_code_dict
                else len(self.feature_code_dict) + 1
                for i in feature_code_raw
            ]
    ...
    def create_country_codes(self, es_data):
        ...
        for ex in es_data:
            country_code_raw = [
                i["country_code3"] for i in ex["es_choices"][0 : self.max_choices]
            ]
    ...
    def create_gaz_features(self, es_data):
        ...
        for ex in es_data:
            alt_name_length = [
                i["alt_name_length"] for i in ex["es_choices"][0 : self.max_choices]
            ]
            ...
            ascii_dist = [
                i["ascii_dist"] for i in ex["es_choices"][0 : self.max_choices]
            ]
            ascii_dist += [99] * (self.max_choices - len(ascii_dist))
            adm1_overlap = [
                i["adm1_count"] for i in ex["es_choices"][0 : self.max_choices]
            ]
            adm1_overlap += [0] * (self.max_choices - len(adm1_overlap))
            country_overlap = [
                i["country_count"] for i in ex["es_choices"][0 : self.max_choices]
            ]
            country_overlap += [0] * (self.max_choices - len(country_overlap))
            in_adm1 = [
                i["admin1_parent_match"] for i in ex["es_choices"][0 : self.max_choices]
            ]
            in_adm1 += [0] * (self.max_choices - len(in_adm1))
            in_country = [
                i["country_code_parent_match"]
                for i in ex["es_choices"][0 : self.max_choices]
            ]
...
class geoparse_model(nn.Module):
    def __init__(
        ...
        logger.debug(
            "Pretrained country embedding dim: {}".format(pretrained_country.shape)
        )
        ...
        self.mix_linear = nn.Linear(
            gaz_feature_count, mix_dim
        )  # number of comparisons --> mix
        self.mix_linear2 = nn.Linear(mix_dim, mix_dim)  # mix --> mix
        self.last_linear = nn.Linear(mix_dim, 1)  # mix --> final
        self.mix_country = nn.Linear(
            pretrained_country.shape[0], pretrained_country.shape[0], bias=False
        )
        self.country_predict = nn.Linear(
            country_size, pretrained_country.shape[0], bias=False
        )
    ...
    def forward(self, input):
        ...
        logger.debug(
            "cos_sim_country: {}, cos_sim_code: {}, cos_sim_doc: {}".format(
                cos_sim_country.shape, cos_sim_country.shape, cos_sim_doc.shape
            )
        )
        ...
        cos_sim_other_locs = torch.unsqueeze(
            torch.transpose(cos_sim_other_locs, 0, 1), 2
        )
        ...
        both_sim = torch.cat(
            (cos_sim_country, cos_sim_code, cos_sim_other_locs, cos_sim_doc, gaz_info),
            2,
        )
        ...
        )  # should be (batch_size, choices)  (44, 25)
