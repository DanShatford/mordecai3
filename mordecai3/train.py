...
def load_data(
    ...
    for source in data_sources:
        ...
        if source == "Prodigy":
            with open(
                f"{data_dir}/pickled_es/es_formatted_prodigy_{max_results}_{limit_types}_fuzzy_{fuzzy}.pkl",
                "rb",
            ) as f:
                es_data = pickle.load(f)
        elif source == "TR":
            with open(
                f"{data_dir}/pickled_es/es_formatted_tr_{max_results}_{limit_types}_fuzzy_{fuzzy}.pkl",
                "rb",
            ) as f:
                es_data = pickle.load(f)
        elif source == "LGL":
            with open(
                f"{data_dir}/pickled_es/es_formatted_lgl_{max_results}_{limit_types}_fuzzy_{fuzzy}.pkl",
                "rb",
            ) as f:
                es_data = pickle.load(f)
        elif source == "GWN":
            with open(
                f"{data_dir}/pickled_es/es_formatted_gwn_{max_results}_{limit_types}_fuzzy_{fuzzy}.pkl",
                "rb",
            ) as f:
                es_data = pickle.load(f)
        elif source == "Synth":
            ...
            with open(
                f"{data_dir}/pickled_es/es_formatted_syn_cities_{max_results}_{limit_types}_fuzzy_{fuzzy}.pkl",
                "rb",
            ) as f:
                es_data_syn1 = pickle.load(f)
            with open(
                f"{data_dir}/pickled_es/es_formatted_syn_caps_{max_results}_{limit_types}_fuzzy_{fuzzy}.pkl",
                "rb",
            ) as f:
            ...
            with open(
                f"{data_dir}/pickled_es/es_formatted_wiki_{max_results}_{limit_types}_fuzzy_{fuzzy}.pkl",
                "rb",
            ) as f:
                es_data = pickle.load(f)
        ...
        es_data = [
            i for i in es_data if len(i["tensor"]) > 1
        ]  # This is really weird!! Some sort of bug in the spacy step
...
def data_formatter_prodigy(docs, data):
    ...
    for doc, ex in tqdm(zip(docs, data), total=len(docs), leave=False):
        ...
        places = [
            i
            for i in doc
            if i.idx >= ex["spans"][0]["start"]
            and i.idx + len(i) <= ex["spans"][0]["end"]
        ]
    ...
    for doc, ex in tqdm(zip(docs, data), total=len(docs), leave=False):
        ...
        if "start_char_sent" in ex.keys():
            orig_places = [
                ent
                for ent in doc.ents
                if ent.start_char >= ex["start_char_sent"]
                and ent.start_char < ex["end_char_sent"]
            ]
        else:
            orig_places = [
                ent
                for ent in doc.ents
                if ent.start_char >= ex["start_char"]
                and ent.start_char < ex["end_char"]
            ]
        # get the tensor for those tokens
        if orig_places:
...
def data_to_docs(data, source, base_dir, nlp):
    ...
    if source in ["prodigy", "syn_cities", "syn_caps", "wiki"]:
        ...
        for doc in tqdm(
            nlp.pipe([i["text"] for i in data["articles"]["article"]]),
            total=len(data["articles"]["article"]),
        ):
            doc_bin.add(doc)
        ...
        for n, topo in enumerate(ex["toponyms"]["toponym"]):
            ...
            try:
                place_tokens = [
                    i
                    for i in doc
                    if i.idx >= int(topo["start"])
                    and i.idx + len(i) <= int(topo["end"])
                ]
...
def format_source(
    ...
    print("Converting back to spaCy docs...")
    ...
    for ff in tqdm(formatted, leave=False):
        esd = es_util.add_es_data_doc(
            ff, conn, max_results, fuzzy, limit_types, remove_correct
        )
        for e in esd:
            if e["correct_geonamesid"] is not None:
                esed_data.append(e)
...
@app.command()
def nlp_docs(base_dir, sources="tr, lgl, gwn, prodigy, syn_cities, syn_caps, wiki"):
    """
...
@app.command()
def train(
    batch_size: int = typer.Option(32, "--batch_size"),  # input batch size for training
    test_batch_size: int = typer.Option(
        64, "--test_batch_size"
    ...
    dataset_names: str = typer.Option(
        "Prodigy, TR, LGL, GWN, Synth, Wiki", "--datasets"
    ),
):
    """
    ...
    train_loader, es_train_data, data_loaders, datasets = load_data(
        data_dir,
        config.max_choices,
    ...
    if config.avg_params:
        ...
        scheduler = optim.lr_scheduler.CosineAnnealingLR(
            optimizer, T_max=config.epochs + 1
        )
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        print(
            f"Epoch {epoch+0:03}: | Loss: {epoch_loss/len(train_loader):.5f} | Exact Match: {wandb_dict['exact_match_avg']:.3f} | Country Match: {wandb_dict['country_avg']:.3f}"
        )  # | Prodigy Acc: {epoch_acc_prod/len(prod_loader):.3f} | TR Acc: {epoch_acc_tr/len(tr_loader):.3f} | LGL Acc: {epoch_acc_lgl/len(lgl_loader):.3f} | GWN Acc: {epoch_acc_gwn/len(gwn_loader):.3f} | Syn Acc: {epoch_acc_syn/len(syn_loader):.3f}')
