...
def doc_to_ex_expanded(doc):
    ...
    loc_ents = [
        ent for ent in doc.ents if ent.label_ in ["GPE", "LOC", "EVENT_LOC", "NORP"]
    ]
    for ent in doc.ents:
        if ent.label_ in ["GPE", "LOC", "EVENT_LOC", "FAC"]:
            ...
            if other_locs:
                locs_tensor = np.mean(
                    np.vstack([i._.tensor for i in other_locs if i not in ent]), axis=0
                )
            else:
                locs_tensor = np.zeros(len(tensor))
            d = {
                "search_name": ent.text,
                "tensor": tensor,
...
class Geoparser:
    def __init__(
        ...
        if check_es:
            try:
                ...
                ConnectionError(
                    "Could not locate Elasticsearch. Are you sure it's running?"
                )
        if not model_path:
            model_path = pkg_resources.resource_filename(
                "mordecai3", "assets/mordecai_2023-03-28.pt"
            )
    ...
    def pick_event_loc(self, d):
        ...
        if not loc_ents:
            if len(geo) == 0:
                ...
                adm1s = list(
                    set(["_".join([i["country_code3"], i["admin1_name"]]) for i in geo])
                )
                soft_locs = [
                    i for i in geo if i["start_char"] in range(loc_start, loc_end + 6)
                ]
                if soft_locs:
                ...
                if soft_loc and re.search(
                    ",|in", d["partial_doc"][loc_end : soft_loc["start_char"]]
                ):
                    d["event_loc"] = soft_loc
                    d[
                        "event_loc_reason"
                    ] = "No event location identified, picking the following comma sep'ed location"
                elif len(set([i["search_name"] for i in geo])) == 1:
                    # note: cities and states could have the same place name
                    d["event_loc"] = geo[0]
                    d[
                        "event_loc_reason"
                    ] = "No event location identified, but only one (unique) location in text"
                # TODO: see if the placenames all fit in a hierarchy (P --> ADM2 --> ADM1) etc.
                    ...
                    d[
                        "event_loc_reason"
                    ] = "No event location identified, using common ADM1"
                    ...
                    d[
                        "event_loc_reason"
                    ] = "No event location identified, using common country"
                # elif:
                #   # see if there's a nearby loc in the sentence here
                    ...
                    d[
                        "event_loc_reason"
                    ] = "Multiple locations, but none identified as the event location"
            else:
                d["event_loc"] = geo[0]
                d[
                    "event_loc_reason"
                ] = "No event location identified, but only one location in text"
        else:
            if len(loc_ents) == 1:
                d["event_loc"] = loc_ents[0]
                d[
                    "event_loc_reason"
                ] = "Match: single geo result overlapping with event location"
            else:
                ...
                if places:
                    ...
                    d[
                        "event_loc_reason"
                    ] = "Multiple locations are within the event location: picking the first non-admin unit"
                else:
                    d["event_loc"] = loc_ents[0]
                    d[
                        "event_loc_reason"
                    ] = "Multiple locations within event location. Picking the first location."
    ...
    def geoparse_doc(
        ...
        if type(text) is str:
            ...
            raise ValueError(
                "Text must be either of type 'str' or 'spacy.tokens.doc.Doc'."
            )

        if plover_cat and not self.event_geoparse:
            raise Warning(
                "A PLOVER category was provided but event geoparsing is disabled. Skipping event geolocation!"
            )

        logger.debug("Doc ents: ", doc.ents)
        doc_ex = doc_to_ex_expanded(doc)
        if doc_ex:
            es_data = add_es_data_doc(
                doc_ex, self.conn, max_results=max_choices, known_country=known_country
            )
        ...
        best_list = []
        output = {"doc_text": doc.text, "event_location": "", "geolocated_ents": []}
        if len(doc_ex) == 0:
            return output
        elif len(es_data) == 0:
            return output
...
if __name__ == "__main__":
    ...
    doc = geo.nlp(
        "Speaking from Berlin, President Obama expressed his hope for a peaceful resolution to the fighting in Homs and Aleppo."
    )
