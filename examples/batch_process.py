    ...
    if not loc_ents:
        if len(geo) == 0:
            ...
            if len(set([i["name"] for i in geo])) == 1:
                ...
                d[
                    "event_loc_reason"
                ] = "No location identified in event location model, but only one (unique) location in text"
            else:
                d["event_loc"] = None
                d[
                    "event_loc_reason"
            ...
            d[
                "event_loc_reason"
            ] = "No location identified in event location model, but only one location in text"
    else:
        if len(loc_ents) == 1:
            ...
            if places:
                ...
                d[
                    "event_loc_reason"
                ] = "Multiple locations within event location. None are feature type places."
    return d
...
def main(
    ...
    if input_format == "icews":
        ...
        raise ValueError(
            "Input format must be one of ['icews', 'production']. 'icews' should be used when coding final, formatted ICEWS event records. 'production' should be used in the actual production pipeline."
        )
    ...
    docs = list(
        tqdm(nlp.pipe(text_list, batch_size=1000, as_tuples=True), total=len(text_list))
    )
    ...
    def resolve_entities(d):
        ...
        for ent, pred in zip(es_data, pred_val):
            ...
            try:
                results = results[0]
                r = {
                    "extracted_name": ent["search_name"],
                    "name": results["name"],
                    "lat": results["lat"],
    ...
    for c in tqdm(range(chunks)):
        ...
        for d in batch:
            q = {
                "question": f"Where did {d['event_text'].lower()} happen?",
                "context": d["text"],
            }
