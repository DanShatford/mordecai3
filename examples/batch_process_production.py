...
def main(input_file: Path, config_file: Path):
    ...
    if limit >= 0:
        logger.info(
            f"CAUTION! A story limit is set. Only processing the first {limit} stories."
        )
    ...
    docs = list(
        tqdm(geo.nlp.pipe(text_list, batch_size=spacy_batch_size), total=len(text_list))
    )
    ...
    for i in story_list:
        for e in i["events"]:
            ...
            if i["geo"]:
                ...
                e["geo"] = [
                    g for g in clean_geo if g["start_char"] < len(e["partial_doc"])
                ]
            else:
                e["geo"] = []
            event_list.append(e)
    ...
    for c in tqdm(range(chunks)):
        ...
        for d in batch:
            q = {
                "question": f"Where did {d['name'].lower()} happen?",  # name = CAMEO code
                # q = {"question": f"Which place did {d['name'].lower()} happen?", # name = CAMEO code
                "context": d["partial_doc"],
            }  # doc up to and including event sentence
